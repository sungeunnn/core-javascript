...
//위의 경우에 first() 는 second()함수 자체를 반환한다.
//따라서 first()() 는 second 함수를 실행



//클로저를 사용하는이유는 전역의 오염을 막기 위해서다.
// 클로저로 사용하지 않은 경우 {skyblue,9}
let count = 0;
...
//이 경우 counts는 함수 안에서 정의되어 있기 때문에 값 자체를 외부에 불러올수없다.
//따라서 전역의 오염을 막을 수 있다.
//또한, 가바지컬렉터가 접근할 수 없다. 자체로 꽁꽁 싸여 있는 너(느)낌~~
//따라서 closure 이다. closure = 폐쇄
